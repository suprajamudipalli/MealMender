document.addEventListener('DOMContentLoaded', () => {
  ...
  async function loadUsers() {
    ...
    users.forEach(user => {
      ...
      let roleButton = '';
      if (user.role === 'admin') {
        // Show demote button, but disable it for the current admin if they are the only one (logic handled by backend, but good for UI)
        roleButton = `<button class="btn btn-sm btn-warning me-2 role-change-btn" data-id="${user._id}" data-role="user" ${isCurrentUser ? 'title="Cannot demote yourself if you are the last admin"' : ''}>Demote</button>`;
      } else {
      ...
      data: {
        ...
        datasets: [{
          ...
          backgroundColor: [
            ...
            'rgba(23, 162, 184, 0.7)', // claimed (blue)
            'rgba(255, 193, 7, 0.7)',  // delivered (yellow)
          ],
          borderColor: '#fff',
          borderWidth: 2
        }]
